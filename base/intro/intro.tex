...
PCA is defined by the transformation $\mathbf{Y}=\mathbf{PX}$ where the $i$th row of $\mathbf{P}$ is the $i$th principal component. The $i$th principal component can be obtained directly as the $i$th eigenvector of the covariance matrix:
...
Here $\mathbf{\Sigma} = \diag(\sigma_{i})$ is a diagonal matrix of of the square roots of the eigenvalues.
...
%There are many algorithms to calculate $\mathbf{P}$; the selection of the largest $i$ is more pertinent for us as it determines the dimensionality of the transformed data.

The principal components have a possible genealogical interpretation\cite{mcvean_genealogical_2009}, reflect geographic patterns across populations\citep{novembre2008europe}, and are effective to correct for population structure in genome wide association studies (GWAS)\cite{price_principal_2006}. PCA is commonly used for visualization, but this requires researchers to specify which two or three dimensions to examine at one time, necessarily excluding much of the data.

% What are PCA's strengths and weaknesses? Not addressed in the QLS comps
